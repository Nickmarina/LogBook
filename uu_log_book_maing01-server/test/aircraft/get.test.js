...
describe(`Testing the ${CMD}...`, () => {
  test("HDS by id", async () => {
    ...
    const result = await TestHelper.executeGetCommand(CMD, { id: aircraft.id }, session);
    expect(result.status).toEqual(200);
    expect(result.data.uuAppErrorMap).toBeDefined();
  });
  test("HDS by regNum", async () => {
    let session = await TestHelper.login("AwidLicenseOwner", false, false);
    let dtoIn = {
      regNum: "kk-jjj",
      model: "model"
    ...
    let aircraft = await TestHelper.executePostCommand("aircraft/create", dtoIn, session);
    const result = await TestHelper.executeGetCommand(CMD, { regNum: aircraft.regNum}, session);
    expect(result.status).toEqual(200);
    expect(result.data.uuAppErrorMap).toBeDefined();
  });
});

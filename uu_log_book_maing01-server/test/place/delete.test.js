...
describe(`Testing the ${CMD}...`, () => {
  test("HDS by id", async () => {
    ...
    const place = await TestHelper.executePostCommand("place/create", dtoIn, session);
    const result = await TestHelper.executePostCommand(CMD, { id: place.id }, session);
